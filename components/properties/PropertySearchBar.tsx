...
interface PropertySearchBarProps {
  /** Default transaction type, synced from the URL segment */
  defaultType?: TransactionType;
  /** Fired when the user submits a search */
  onSearch?: (filters: PropertyFilter) => void;
}

// ─── Constants (match PropertyFilterBar) ────────────────────────────────────────
...
// ─── FilterDropdown (match PropertyFilterBar) ───────────────────────────────────
...
const PropertySearchBar: React.FC<PropertySearchBarProps> = ({
  ...
  // ── Scroll / sticky state (KEEP: PropertySearchBar-specific) ─────────────────
  const [isSticky, setIsSticky] = useState(false);
  const [isHiddenByFooter, setIsHiddenByFooter] = useState(false);
  const sentinelRef = useRef<HTMLDivElement>(null);
  const barRef = useRef<HTMLDivElement>(null);

  const transactionType = (searchParams.get("transactionType") as TransactionType) ?? defaultType;
  ...
  useEffect(() => {
    ...
    if (!footer) return;
    const io = new IntersectionObserver(
      ([entry]) => setIsHiddenByFooter(entry.isIntersecting),
      { threshold: 0.05 }
    );
    io.observe(footer);
    return () => io.disconnect();
  ...
  const buildFilterUrl = useCallback(
    (overrides?: { q?: string; type?: string; min?: string; max?: string }) => {
      ...
      if (transactionType !== defaultType)
        params.set("transactionType", transactionType);
      const query = params.toString();
      return `/properties/${transactionType}/in-dubai${query ? `?${query}` : ""}`;
    },
    [searchQuery, propertyType, minPrice, maxPrice, transactionType, defaultType]
  );
  ...
  const applyFilterChange = useCallback(
    (updates: { q?: string; type?: string; min?: string; max?: string }) => {
  ...
  // ── Search input (match PropertyFilterBar) ──────────────────────────────────
  const searchInput = (
    <>
      <label htmlFor="property-search-bar" className="sr-only">
  ...
  // ── Bar content (match PropertyFilterBar layout) ──────────────────────────────
  const barContent = (
    <form onSubmit={handleSearch} className="w-full">
      ...
      <div className="relative flex items-center gap-2 lg:hidden">
        ...
        >
          ...
          {hasActiveFilters && (
            <span
              className="absolute -right-0.5 -top-0.5 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-[var(--charcoal)]"
              aria-hidden
            />
          )}
        </button>
              ...
              <div className="space-y-5 p-5">
                ...
                <div className="relative" ref={listingPanelRefMobile}>
                  <button
                    ...
                    aria-label={`Listing type: ${transactionType === "rent" ? "Rent" : "Buy"}`}
      ...
      <div className="hidden lg:flex lg:flex-row lg:items-center lg:gap-3">
        <div className="relative shrink-0" ref={listingPanelRef}>
          ...
          {listingDropdownOpen && (
            ...
            >
              <li role="option" aria-selected={transactionType === "rent"}>
                <Link
                  href="/properties/rent/in-dubai"
              ...
              <li role="option" aria-selected={transactionType === "buy"}>
                <Link
                  href="/properties/buy/in-dubai"
                  className="block px-4 py-2.5 text-sm font-medium transition-colors hover:bg-[var(--soft-sand)]/30"
                  style={{
  ...
  return (
    <>
      ...
      >
        <section
          className="py-4 md:py-6 shadow-[0_1px_0_0_rgba(0,0,0,0.05)]"
